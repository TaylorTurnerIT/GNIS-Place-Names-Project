...
print(f"\n=== COUNTY ANALYSIS ===")
place_counties = place_names['County'].value_counts().head(10)
gnis_counties = gnis['county_name'].value_counts().head(10)
print(f"\nTop 10 PlaceNames counties:\n{place_counties}")
print(f"\nTop 10 GNIS counties:\n{gnis_counties}")

# Analyze feature classes in GNIS
print(f"\n=== GNIS FEATURE CLASSES ===")
feature_classes = gnis['gaz_featureclass'].value_counts().head(20)
print(feature_classes)

# Check for (historical) markers
print(f"\n=== HISTORICAL MARKERS ===")
gnis_historical = gnis['gaz_name'].str.contains(r'\(historical\)', case=False, na=False).sum()
...
place_suffixes = get_suffixes(place_names['Place_Name'])
for suffix, count in place_suffixes:
    print(f"  {suffix}: {count}")

print("\nCommon suffixes in GNIS:")
gnis_suffixes = get_suffixes(gnis['gaz_name'])
...
sample_names = ['Aaron', 'Abbott', 'Abiff', 'Abernathy']
for name in sample_names:
    print(f"\n'{name}' variations in GNIS:")
    matches = gnis[gnis['gaz_name'].str.contains(name, case=False, na=False)]['gaz_name'].head(5)
    if len(matches) > 0:
